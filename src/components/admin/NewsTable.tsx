...
export default function NewsTable() {
  ...
  function load() {
    ...
    setArticles(all.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
  }
  ...
  return (
    <div className="space-y-6 max-w-[1400px]">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-bold tracking-tight text-secondary">News</h1>
          <p className="text-sm text-muted-foreground mt-1">Manage articles</p>
        </div>
        <Link href="/admin/news/new" className="flex items-center gap-2 h-11 px-5 rounded-2xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 active:scale-[0.97] transition-all">
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center bg-card rounded-3xl border border-border/50">
          ...
          {!search && (
            <Link href="/admin/news/new" className="mt-5 flex items-center gap-2 h-10 px-5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 active:scale-[0.97] transition-all">
                ...
                {filtered.map(a => (
                  <tr key={a.id} className="border-b border-border/20 last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-4 hidden sm:table-cell">
                      <span className="inline-block px-2.5 py-1 rounded-lg bg-muted text-xs font-medium text-muted-foreground">{a.category}</span>
                    </td>
