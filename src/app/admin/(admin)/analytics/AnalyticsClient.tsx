...
export default function AnalyticsClient() {
  ...
  const [activityBreakdown, setActivityBreakdown] = useState<{ action: string; count: number }[]>([]);
  const [visitorStats, setVisitorStats] = useState<{ totalViews: number; uniqueVisitors: number; todayViews: number; topPages: { path: string; count: number }[] } | null>(null);
  ...
  if (!hasPermission) return <Spinner />;
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-card rounded-2xl border border-border/50 p-5">
                ...
                <p className="font-display text-3xl font-bold text-secondary">
                  {contentData.reduce((s, c) => s + (c.count ?? 0), 0).toLocaleString()}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-card rounded-2xl border border-border/50 p-5">
                ...
                <div className="h-[260px]">
                  {contentData.some(d => (d.count ?? 0) > 0) ? <ContentBreakdownBar data={contentData} /> : <EmptyChart />}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-card rounded-2xl border border-border/50 p-5">
                ...
                <div className="h-[260px]">
                  {visitorStats && visitorStats.topPages.length > 0 ? <TopPagesBar data={visitorStats.topPages} /> : <EmptyChart />}
                </div>
              </div>
            </div>
          </>
