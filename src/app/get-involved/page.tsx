  ...
  return (
    <main suppressHydrationWarning className="bg-[#fafbf9]">
      <section className="relative min-h-[50dvh] flex items-end pb-12 pt-32 overflow-hidden bg-white">
        <div className="absolute inset-0 bg-[#0a2e1c]/5" style={{ backgroundImage: 'radial-gradient(#0a2e1c 0.5px, transparent 0.5px)', backgroundSize: '30px 30px' }} />
        <div className="max-w-7xl mx-auto px-6 w-full relative z-10 text-center md:text-left">
          ...
          >
            ...
            <h1 className="font-display text-[clamp(2.5rem,8vw,5rem)] font-extrabold text-[#0a2e1c] tracking-tighter leading-[0.9]">
              Empower <span className="text-green italic font-light serif">The Future</span>.
            </h1>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {ways.map((way, i) => (
              <BentoCard
                key={way.id}
                delay={i * 0.1}
                className="flex flex-col h-full bg-white border-none shadow-sm hover:shadow-xl transition-all"
          ...
          <div className="bg-white p-8 md:p-12">
            <div className="flex items-start justify-between mb-8">
              <div>
                ...
                <h2 className="font-display text-4xl font-extrabold text-deep tracking-tighter leading-none">
                  {selectedWay.title}
                </h2>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <div className="space-y-6">
                 <div>
                    <h4 className="font-bold text-deep uppercase text-[10px] tracking-widest mb-4">What to Expect</h4>
                    <div className="space-y-3">
                      {selectedWay.details.split("|").map((detail: string, i: number) => (
                        <div key={i} className="flex items-center gap-3 text-sm text-slate-600 bg-slate-50 px-4 py-3 rounded-xl border border-slate-100/50">
              ...
              <div className="bg-deep rounded-[2.5rem] p-8 text-white relative overflow-hidden">
                  ...
                  <div className="space-y-6 mb-8 relative z-10">
                    ...
                    {donateAmount === "custom" && (
                      <input
                        ...
                        className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-gold/50 transition-all"
                        value={customAmount}
                ...
                <form className="space-y-4 relative z-10" onSubmit={selectedWay.id === "donate" ? handleDonate : (e) => { e.preventDefault(); alert("Success!"); setSelectedWay(null); }}>
                  ...
                  <button className="w-full py-4 bg-gold text-deep font-bold rounded-xl text-sm hover:bg-white transition-all flex items-center justify-center gap-2 mt-4 shadow-lg shadow-gold/10">
