...
export default function BlogPost({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main suppressHydrationWarning className="bg-[#fafbf9]">
      ...
      <section className="relative pt-24 md:pt-32 pb-12 md:pb-20 px-6 overflow-hidden bg-white">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-20 items-center relative z-10">
          <div className="lg:col-span-7 text-center lg:text-left order-2 lg:order-1">
            ...
            >
              <Link href="/news" className="hidden lg:inline-flex items-center gap-2 text-slate-400 hover:text-green text-xs font-bold uppercase tracking-widest mb-8 transition-colors group">
              ...
              <h1 className="font-display text-[clamp(2rem,8vw,4.5rem)] font-extrabold text-deep tracking-tighter leading-[0.95] mb-6 lg:mb-8">
                {article.title}
              </h1>

              <p className="text-slate-500 text-base md:text-lg leading-relaxed max-w-2xl mx-auto lg:mx-0 italic border-l-0 lg:border-l-4 border-gold lg:pl-6">
                {article.desc}
              </p>
            </motion.div>
          </div>
          ...
          <article className="lg:col-span-8">
            <div className="prose prose-slate lg:prose-lg max-w-none">
              {article.content.split('\n\n').map((paragraph, i) => (
                <p key={i} className="text-slate-600 text-base md:text-lg leading-[1.8] mb-8 first-letter:text-4xl md:first-letter:text-5xl first-letter:font-bold first-letter:text-deep first-letter:mr-2 md:first-letter:mr-3 first-letter:float-left">
                  {paragraph}
                </p>
              ))}
            </div>
            ...
            {/* Engagement Footer */}
            <div className="mt-12 md:mt-16 pt-8 border-t border-slate-100 flex flex-wrap items-center justify-between gap-6">
               <div className="flex items-center gap-4">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Share story</span>
                  <div className="flex gap-2">
                     <button className="w-9 h-9 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-deep hover:text-white transition-all"><Share2 size={16} /></button>
                     <button className="w-9 h-9 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-deep hover:text-white transition-all"><Bookmark size={16} /></button>
                  </div>
               </div>
               <Link href="/get-involved" className="text-xs md:text-sm font-bold text-green hover:gap-3 transition-all flex items-center gap-2">
                  Be part of the next story <ArrowLeft size={16} className="rotate-180 text-gold" />
               </Link>
            </div>
          </article>
          ...
          <aside className="lg:col-span-4 space-y-8 md:space-y-12">
            <div className="sticky top-32 space-y-8 md:space-y-12">
               ...
               <div className="bg-deep rounded-[2.5rem] p-8 md:p-10 text-white relative overflow-hidden shadow-2xl">
                  ...
                  <Link href="/programs" className="block w-full mt-10 py-4 bg-white/5 border border-white/10 rounded-2xl text-center text-[10px] font-bold hover:bg-white hover:text-deep transition-all uppercase tracking-widest">
                     View All Programs
                  </Link>
               </div>

               {/* Newsletter Box */}
               <div className="bg-gold rounded-[2.5rem] p-8 md:p-10 text-deep shadow-xl text-center md:text-left">
                  <h3 className="font-display text-xl md:text-2xl font-bold mb-4">Stay Notified.</h3>
                  <p className="text-deep/70 text-xs md:text-sm leading-relaxed mb-8">
                  ...
                  <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); alert("Subscribed!"); }}>
                     <input type="email" placeholder="Your Email" className="w-full px-5 py-4 bg-white/30 border border-deep/10 rounded-xl text-sm placeholder:text-deep/40 focus:outline-none" required />
                     <button className="w-full py-4 bg-deep text-white rounded-xl text-sm font-bold shadow-lg">Join 500+ Readers</button>
                  </form>
               </div>
            </div>
          </aside>
      ...
      {relatedStories.length > 0 && (
        <section className="py-20 md:py-24 px-6 bg-slate-50/50 border-t border-slate-100">
          <div className="max-w-7xl mx-auto text-center md:text-left">
              <h2 className="font-display text-2xl md:text-3xl font-extrabold text-deep tracking-tight mb-12">More from the Chronicle</h2>
                ...
                {relatedStories.map((item, i) => (
                  <FadeIn key={i} delay={i * 0.1}>
                      <Link href={`/news/${item.id}`} className="group block">
                        ...
                        <div className="flex items-center justify-center md:justify-start gap-3 mb-3 text-[9px] font-bold uppercase tracking-widest text-slate-400">
                            <span className="text-gold">{item.category}</span>
                            <span>{item.date}</span>
                        </div>
                        <h3 className="font-display text-lg font-bold text-deep group-hover:text-green transition-colors leading-tight line-clamp-2">
                            {item.title}
                        </h3>
                      </Link>
                  </FadeIn>
