          ...
          <article className="lg:col-span-8">
            <div className="prose prose-slate lg:prose-lg max-w-none">
              {article.content.split('\n\n').map((paragraph, i) => (
                <p key={i} className="text-muted-foreground text-base md:text-lg leading-[1.8] mb-8 first-letter:text-4xl md:first-letter:text-5xl first-letter:font-bold first-letter:text-secondary first-letter:mr-2 md:first-letter:mr-3 first-letter:float-left">
                  {paragraph}
                </p>
              ))}
            </div>
            ...
            {/* Engagement Footer */}
            <div className="mt-12 md:mt-16 pt-8 border-t border-border/50 flex flex-wrap items-center justify-between gap-6">
               <div className="flex items-center gap-4">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Share story</span>
                  <div className="flex gap-2">
                     <button className="w-9 h-9 rounded-xl bg-muted flex items-center justify-center text-muted-foreground hover:bg-secondary hover:text-secondary-foreground transition-all"><Share2 size={16} /></button>
                     <button className="w-9 h-9 rounded-xl bg-muted flex items-center justify-center text-muted-foreground hover:bg-secondary hover:text-secondary-foreground transition-all"><Bookmark size={16} /></button>
                  </div>
               </div>
          ...
          <aside className="lg:col-span-4 space-y-8 md:space-y-12">
            <div className="sticky top-32 space-y-8 md:space-y-12">
               ...
               <div className="bg-secondary rounded-bento p-8 md:p-10 text-secondary-foreground relative overflow-hidden shadow-2xl">
                  ...
                  <Link href="/programs" className="block w-full mt-10 py-4 bg-card/5 border border-card/10 rounded-2xl text-center text-[10px] font-bold hover:bg-card hover:text-secondary transition-all uppercase tracking-widest">
                     View All Programs
                  </Link>
               </div>

               {/* Newsletter Box */}
               <div className="bg-accent rounded-bento p-8 md:p-10 text-accent-foreground shadow-xl text-center md:text-left">
                  ...
                  <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); alert("Subscribed!"); }}>
                     <input type="email" placeholder="Your Email" className="w-full px-5 py-4 bg-card/30 border border-accent-foreground/10 rounded-xl text-sm placeholder:text-accent-foreground/40 focus:outline-none" required />
                     <button className="w-full py-4 bg-secondary text-secondary-foreground rounded-xl text-sm font-bold shadow-lg">Join 500+ Readers</button>
                  </form>
               </div>
            </div>
          </aside>
