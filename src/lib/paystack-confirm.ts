...
export async function confirmChargeSuccess(data: ChargeSuccessData): Promise<string> {
...
export async function verifyPaystackTransaction(reference: string): Promise<{
  ...
  if (!secret) {
    return { status: "unconfigured", error: "Payments are not configured." };
  }
  ...
  const body: { status: boolean; data?: Record<string, unknown> } = await res.json();
  if (body.status && body.data?.status === "success") {
    const d = body.data;
    const customer = d.customer as { email?: string } | undefined;
    await confirmChargeSuccess({
      reference: (d.reference as string) || reference,
      amount: d.amount as number | undefined,
      currency: d.currency as string | undefined,
      customer: { email: customer?.email || "" },
      metadata: (d.metadata as PaystackMetadata | undefined) || {},
    });
